...
struct ConnectionState {
    pending: HashMap<SocketAddr, IncomingConnection>,
    accepting: std::collections::HashSet<SocketAddr>,
    active: std::collections::HashSet<SocketAddr>,
}
...
pub struct KcpListener {
    ...
    active_streams: Arc<DashMap<SocketAddr, StreamHandle>>,
...
impl KcpListener {
    ...
    pub async fn bind(addr: SocketAddr, config: KcpConfig) -> Result<Self> {
        let transport = crate::transport::UdpTransport::bind(addr)
            .await
            .map_err(KcpError::Io)?;
    ...
    pub async fn with_transport(transport: Arc<dyn Transport>, config: KcpConfig) -> Result<Self> {
        ...
        loop {
            if let Some(incoming) = self.connection_queue.recv().await {
                // Mark as accepting to prevent duplicate processing
                {
                    ...
                    if state.active.contains(&incoming.peer_addr) {
                        info!(peer = %incoming.peer_addr, "Ignoring connection attempt from active peer");
                        continue;
                    }

                    state.accepting.insert(incoming.peer_addr);
                }
                ...
                match self.create_stream_for_connection(incoming.clone()).await {
                    Ok(stream) => {
                        {
                            let mut state = self.conn_state.write().await;
                            state.accepting.remove(&incoming.peer_addr);
                        }
    ...
    async fn handle_incoming_packet(
        ...
        if data.len() >= KcpHeader::SIZE {
            ...
            if let Some(header) = KcpHeader::decode(&mut data_clone) {
                ...
                state.pending.insert(peer_addr, incoming.clone());
                drop(state);
    ...
    async fn create_stream_for_connection(
        ...
        {
            let mut state = self.conn_state.write().await;
            state.pending.remove(&incoming.peer_addr);
            state.active.insert(incoming.peer_addr);
        }
        self.active_streams.insert(incoming.peer_addr, handle);
    ...
    async fn cleanup_dead_streams(
        ...
        let dead_addrs: Vec<SocketAddr> = active_streams
            .iter()
