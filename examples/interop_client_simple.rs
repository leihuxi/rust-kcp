...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        loop {
            match socket_recv.recv(&mut buf).await {
                Ok(size) => {
                    ...
                    // Flush any output generated by input processing
                    for out in engine.drain_output() {
                        let _ = socket_recv.send(&out).await;
                    }
                }
                Err(e) => {
    ...
    let socket_update = socket.clone();
    let update_task = tokio::spawn(async move {
        ...
        loop {
            ...
            for out in engine.drain_output() {
                let _ = socket_update.send(&out).await;
            }
        }
    });
    ...
    for (i, message) in test_messages.iter().enumerate() {
        ...
        // Send message
        {
            ...
            match engine.send(Bytes::from(message.as_bytes())) {
                Ok(()) => {
