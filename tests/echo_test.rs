...
async fn test_echo_server_client() {
    ...
    match result {
        Ok(Ok(n)) => {
            let response = &buf[..n];
            println!("Client: Received echo '{}'", String::from_utf8_lossy(response));
...
async fn test_multiple_messages() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        for i in 0..3 {
            let n = stream.read(&mut buf).await.expect("Failed to read");
            println!("Server: Message {}: '{}'", i+1, String::from_utf8_lossy(&buf[..n]));
            stream.write_all(&buf[..n]).await.expect("Failed to write");
            stream.flush().await.expect("Failed to flush");
        }
    });
    ...
    for msg in &messages {
        // Send
        client.write_all(msg.as_bytes()).await.expect("Failed to send");
        client.flush().await.expect("Failed to flush");
