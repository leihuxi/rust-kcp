...
async fn test_exact_kcp_socket_pattern() {
    ...
    let server_socket = UdpSocket::bind(server_addr).await.expect("Failed to bind server");
    ...
    let server_task = tokio::spawn(async move {
        ...
        for i in 0..5 { // Try receiving a few packets
            match tokio::time::timeout(Duration::from_millis(500), server_socket_clone.recv_from(&mut buf)).await {
                Ok(Ok((size, peer_addr))) => {
                    println!("Server: [{}] Received {} bytes from {}: {:02x?}", 
                             i, size, peer_addr, &buf[..size.min(16)]);
                }
                Ok(Err(e)) => {
    ...
    let client_socket = UdpSocket::bind("0.0.0.0:0").await.expect("Failed to bind client");
    client_socket.connect(server_addr).await.expect("Failed to connect client");
    println!("Client bound to {} and connected to {}", 
             client_socket.local_addr().unwrap(), server_addr);
